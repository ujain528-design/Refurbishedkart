...
import { ChevronDown, ArrowRight, LogoMark } from "@/components/Icons";
...
function MegaDropdown({ category }) {
  const base = `/products/${category.name.toLowerCase()}`;
  return (
    <div
      className="invisible absolute left-1/2 top-full z-50 w-[420px] -translate-x-1/2 translate-y-3 pt-3 opacity-0 transition-all duration-300 ease-out group-hover:visible group-hover:translate-y-0 group-hover:opacity-100"
    >
      <div className="overflow-hidden rounded-card border border-black/5 bg-white shadow-card-hover">
        <div className="flex items-center justify-between border-b border-black/5 px-6 py-4">
          <p className="text-sm font-semibold text-ink">
            Refurbished {category.name}
          </p>
          <Link
            href={base}
