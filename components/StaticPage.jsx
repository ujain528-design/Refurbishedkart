      ...
      <main>
        <section className="border-b border-black/5 bg-offwhite">
          <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
            ...
            {subtitle && <p className="mt-3 text-[15px] text-neutral-500">{subtitle}</p>}
          </div>
        </section>
...
export function H2({ children }) {
  return <h2 className="mt-9 text-xl font-bold text-ink first:mt-0">{children}</h2>;
}
export function P({ children }) {
  return <p className="mt-3 text-[15px] leading-relaxed text-neutral-600">{children}</p>;
}
export function UL({ items }) {
  return (
    <ul className="mt-3 space-y-2">
      {items.map((it, i) => (
        <li key={i} className="flex items-start gap-2.5 text-[15px] leading-relaxed text-neutral-600">
