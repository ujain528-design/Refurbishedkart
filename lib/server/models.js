...
const OrderSchema = new Schema(
  {
    ...
    // Per-item device serial numbers, captured (mandatory) at ship time. One entry
    // per order line; drives the invoice's Serial No. column + serial list.
    serialNumbers: {
      type: [{ productId: Number, productName: String, variant: String, serialNumber: String }],
      default: [],
    },
    // Invoice-updated notice (e.g. serial numbers added post-shipping). Drives the
    // customer's floating account notification + the "download to dismiss" flow.
    invoiceUpdatedAt: Date,
    invoiceUpdateMessage: String,
    invoiceUpdateSeen: { type: Boolean, default: false },
    customerName: String,
    // Razorpay payment
...
const CouponSchema = new Schema(
  {
    ...
    value: { type: Number, default: 0 },       // % or ₹ amount (0 for free_shipping)
    maxDiscount: { type: Number, default: 0 },  // cap on percent discount (0 = no cap)
    minSubtotal: { type: Number, default: 0 },
    // Usage
    usageLimit: Number,                          // total uses (0/undefined = unlimited)
    perCustomerLimit: { type: Number, default: 1 }, // per-customer (0 = unlimited)
    used: { type: Number, default: 0 },
    // Validity window. `expiry` kept as a legacy alias so old coupons still validate.
    startDate: Date,
    expiryDate: Date,
    expiry: Date,
    ...
    customerSegment: {
      type: String,
      enum: ["all", "first_order", "nth_order", "returning", "new_signup", "high_value", "inactive", "whatsapp", "specific"],
      default: "all",
    },
    nthOrder: { type: Number, default: 0 },
    newSignupDays: { type: Number, default: 0 },
    highValueAmount: { type: Number, default: 0 },
    inactiveDays: { type: Number, default: 0 },
    allowedEmails: { type: [String], default: [] },
    allowedPhones: { type: [String], default: [] },
    // Product/category restrictions (empty = all)
    applicableCategories: { type: [String], default: [] },
    applicableBrands: { type: [String], default: [] },
    // Auto-apply (no code entry needed)
...
const ReviewSchema = new Schema(
  {
    ...
    productName: String,
    reviewer: String,
    userId: { type: String, index: true }, // customer who wrote it (for dedupe + verify)
    title: String,
    rating: { type: Number, min: 1, max: 5 },
    text: String,
    images: { type: [String], default: [] }, // uploaded review photos + selected product images
...
/* Admin-managed homepage builder sections (drive the DB zone on the homepage). */
const HomepageSectionSchema = new Schema(
  {
    type: { type: String, enum: ["product_row", "banner", "category_grid", "announcement"], required: true },
    title: String,
