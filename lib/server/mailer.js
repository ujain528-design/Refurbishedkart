...
/* Invoice-updated notice — sent to the customer when an admin adds/edits the
   device serial numbers (or otherwise updates) their invoice after shipping. */
export async function sendInvoiceUpdatedEmail(to, order = {}) {
  if (!to) return null;
  const from = process.env.EMAIL_FROM || process.env.EMAIL_USER;
  const name = order.customerName || order.shippingAddress?.name || "there";
  const ref = order.orderId || order.id || "";
  const subject = `Invoice Updated - Order #${ref} | RefurbishedKart`;
  const body = `<p style="font-size:14px;color:#333">Hi ${name},</p>
    <p style="font-size:14px;color:#333">Your invoice for Order <b>#${ref}</b> has been updated with device serial numbers.</p>
    <p style="font-size:14px;color:#333">Please download your updated invoice from your account page:<br>
      <a href="https://refurbishedkart.com/account" style="color:#1B5E20;font-weight:600">https://refurbishedkart.com/account</a></p>
    <p style="font-size:13px;color:#666">Questions? +91 8448296273</p>`;
  return mailer().sendMail({ from, to, subject, text: body.replace(/<[^>]+>/g, " "), html: wrap(body) });
}
