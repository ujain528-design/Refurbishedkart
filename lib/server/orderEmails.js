...
// Recipient resolution: prefer the address email, then any order-level email, then
// fall back to the customer's account email (saved addresses may not carry one).
// Async because the last step reads the User document.
const emailOf = async (order) => {
  if (order?.shippingAddress?.email) return order.shippingAddress.email;
  if (order?.email) return order.email;
  if (order?.userId) {
    const user = await User.findById(order.userId).select("email").lean().catch(() => null);
...
const send = async (order, subject, headline, bodyHtml, textBody) => {
  const to = await emailOf(order);
  if (!to) return null;
