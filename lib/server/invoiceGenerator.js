...
export async function generateInvoice(order) {
  ...
  // Backfill missing line categories from the Product (older orders never stored a
  // category on the line), so the "Refurbished [Category] - [Name]" prefix works
  // retroactively. In-memory only; matched by numeric productId (or _id fallback).
  for (const l of order.lines || []) {
    if (!l.category) {
      const key = l.productId ?? l._id;
      if (key != null) {
        // eslint-disable-next-line no-await-in-loop -- few lines per order
        const p = await Product.findOne({ id: key }).select("category").lean();
