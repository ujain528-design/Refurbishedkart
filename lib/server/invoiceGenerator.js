...
export async function generateInvoice(order) {
  ...
  gross.lines.forEach((l, i) => {
    ...
    // Serial No. shown as a sub-line under the product (the table is too narrow for a
    // dedicated column; the full list is also printed below the items).
    const serial = order.serialNumbers?.[i]?.serialNumber || "";
    const descText = `${l.name}${variant ? ` (${variant})` : ""}${serial ? `\nSerial No: ${serial}` : ""}`;
