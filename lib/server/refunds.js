...
export function lineRefundBasis(order, line) {
  ...
  // COD: only grant the FULL basis when the balance collection is EXPLICITLY
  // confirmed (codBalanceCollected === true, set when the admin ticks the box at
  // delivery). Otherwise — not yet delivered, OR delivered but unconfirmed (legacy
  // orders, or a status change that skipped the checkbox) — cap at this line's
  // proportional share of codUpfront, so the refund default never exceeds what's
  // confirmed collected. Admin can still raise it via the over-refund acknowledgment
  // after verifying delivery records.
  const isCod = String(order?.paymentMethod || "").toUpperCase() === "COD";
  const codFullyCollected = order?.codBalanceCollected === true;
  if (isCod && !codFullyCollected) {
