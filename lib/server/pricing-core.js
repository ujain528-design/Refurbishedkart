...
/* Listed Price + additional component cost.
   RAM upgrade charges only the EXTRA capacity (target − default) as a module of
   the default RAM type. SSD upgrade charges the full price of the new capacity. */
export function calculateUpgradePrice(product, ramCapacity, ssdCapacity, cfg) {
  ...
  if (ramCapacity && dRam.capacity && String(ramCapacity) !== String(dRam.capacity)) {
    const extra = ramGB(ramCapacity) - ramGB(dRam.capacity);
    if (extra > 0) price += priceForExtraCapacity(extra, dRam.type, cfg); // decomposes non-standard sizes
  }
  if (ssdCapacity && dSsd.capacity && String(ssdCapacity) !== String(dSsd.capacity)) {
    price += getSsdPrice(ssdCapacity, cfg);
  }
  return price;
}
...
export function recomputeProductPricing(p, cfg) {
  ...
  const ramCost = p.defaultRam.isOnboard ? Number(p.defaultRam.cost || 0)
