...
// Builds product docs reusing the pricing engine (listed = deviceCost + RAM + SSD)
// and the SEO-title path. The route re-validates on import, so the preview can't
// be bypassed.
...
export function buildProductDoc(category, rec, ctx, id) {
  ...
  const deviceCost = Number(val(rec, "deviceCost")) || 0;
  ...
  const listedPrice = deviceCost + ramCost + ssdCost;
  ...
  const TOP = new Set(["brand", "model", "deviceCost", "mrp", "chassisStock", "condition", "warrantyPeriod", "hsnCode", "description", "seoTitle", "category", "workstationTag"]);
