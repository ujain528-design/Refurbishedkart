...
export function buildProductDoc(category, rec, ctx, id) {
  ...
  // Actual RAM type from the row (DDR3/DDR4/DDR5/LPDDR…) so the stored default RAM
  // + its config carry the real type, not a hardcoded "DDR4". (Cost lookup below
  // is intentionally left on the DDR4 table — pricing behaviour unchanged.)
  const ramType = (hasRam && val(rec, "ramType")) || "DDR4";
  const ramCost = ramCap ? getPriceForCapacity(ramGB(ramCap), "DDR4", ctx.cfg) : 0;
  const ssdCost = ssdCap ? getSsdPrice(ssdCap, ctx.cfg) : 0;
  ...
  const TOP = new Set(["brand", "model", "deviceCost", "mrp", "chassisStock", "condition", "warrantyPeriod", "hsnCode", "description", "whats_in_box", "seoTitle", "category", "workstationTag", "processor_count"]);
  ...
  if (mrp) doc.mrp = mrp;
  // Physical CPU count — top-level field (servers/workstations column); default 1.
  doc.processorCount = Math.max(1, Math.round(Number(val(rec, "processor_count")) || 1));
  const seo = val(rec, "seoTitle");
  if (seo) doc.seoTitle = seo;

  if (hasRam) doc.defaultRam = { capacity: ramCap, type: ramType, isOnboard: /LPDDR/i.test(ramType), cost: ramCost };
  if (hasSsd) doc.defaultSsd = { capacity: ssdCap, cost: ssdCost };
  if (hasRam && hasSsd) {
    doc.configs = [{ ram: `${ramCap} ${ramType}`.trim(), ssd: ssdCap, isDefault: true, price: listedPrice, available: true, show: true }];
  }
  return doc;
}
