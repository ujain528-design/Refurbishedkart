...
/* Deterministic shuffle from a string seed (mulberry32-style RNG over a hashed
   seed). Same seed → same order, so a DAILY seed keeps the product order stable
   for the whole day for every visitor (no "reorder on back button"), then
   reshuffles automatically the next day. */
...
export async function queryProducts(params = {}) {
  ...
  const { tags, category, brand, minPrice, maxPrice, exclude, q, limit, sort } = params;
  ...
  if (brand) {
    ...
    // Match by brand field (case-insensitive) OR the brand appearing as a whole
    // word in the product name (covers rows with a missing/blank brand field).
    const word = new RegExp(`\\b${b.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i");
    rows = rows.filter((p) => String(p.brand || "").toLowerCase() === b || word.test(String(p.name || "")));
  }
  ...
  // Default ordering is a DAILY-SEEDED shuffle so products are mixed across
  // brands (instead of clumping by upload order) while staying STABLE for the
  // whole day — every visitor sees the same order, with no "reorder on back
  // button" weirdness; it reshuffles automatically the next day.
  // Bypassed when: an explicit `sort` is passed (listing re-sorts client-side),
  // OR it's a search (`q`) — search must stay in deterministic relevance/id
  // order, never shuffled. Shuffle runs BEFORE the limit so homepage sections
  // (sliced by limit) also draw from the mixed order.
  if (sort || q) {
    rows.sort((a, b) => a.id - b.id);
  } else {
    rows = seededShuffle(rows, new Date().toDateString());
  }
...
export async function productReviews(id) {
  ...
  const reviews = docs.map((r) => ({ id: String(r._id), name: r.reviewer, reviewer: r.reviewer, rating: r.rating, title: r.title || "", text: r.text, images: Array.isArray(r.images) ? r.images : [], date: r.createdAt }));
