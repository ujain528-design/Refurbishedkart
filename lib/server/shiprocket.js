...
export async function createOrder(order) {
  ...
  // The coupon discount is ALREADY baked into the amounts we send Shiprocket
  // (order.total for prepaid, order.codRemaining for COD). So we must NEVER send it
  // again as total_discount / per-line discount — that made Shiprocket deduct it a
  // second time (e.g. collect 18000 − 499 = 17501 instead of 18000). Instead we bake
  // the discount into each line's selling_price so the line items are self-consistent.
  const lines = order.lines || [];
  const discount = Number(order.discount) || 0;
  const subtotalPre = lines.reduce((a, l) => a + (Number(l.unitPrice) || 0) * (Number(l.qty) || 1), 0);
  const discountedUnit = (l) => {
    const unit = Number(l.unitPrice) || 0;
    const qty = Number(l.qty) || 1;
    if (!discount || subtotalPre <= 0) return unit;
    const lineDiscount = Math.round((discount * unit * qty) / subtotalPre); // proportional
    return Math.max(0, Math.round(unit - lineDiscount / qty));
  };
  ...
  const payload = {
    ...
    order_items: lines.map((l) => ({
      ...
      // Discounted unit price (coupon baked in) — never send `discount` separately.
      selling_price: discountedUnit(l),
      discount: 0,
      tax: "",
      hsn: l.hsnCode || "",
    })),
    ...
    // Discount is already reflected in sub_total + line selling_price — sending it
    // again here caused Shiprocket to deduct it twice. Always 0.
    total_discount: 0,
