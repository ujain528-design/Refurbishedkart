...
// Best auto-apply coupon for the signed-in customer (or null). Never throws —
// unauthenticated / no-match / network error all resolve to null.
export const getAutoCoupon = (subtotal, category = "") =>
  get(`/api/coupons/auto?subtotal=${encodeURIComponent(subtotal || 0)}&category=${encodeURIComponent(category)}`)
    .then((r) => r.coupon || null)
    .catch(() => null);
