...
// Distinct brands that have products (for the dynamic brand strip).
export const getBrands = () => get("/api/brands").then((r) => r.brands || []);

// Reviews
export const getReviews = (productId) => get(`/api/reviews/product/${productId}`);
// Can the current customer review this product? (never throws → login state)
export const getReviewEligibility = (productId) =>
  get(`/api/reviews?productId=${encodeURIComponent(productId)}`).catch(() => ({ loggedIn: false }));
// Submit a verified-purchase review (multipart: fields + optional photos).
export const submitReview = (formData) => uploadForm("/api/reviews", formData);

// Homepage builder — public (active sections) + admin (all + publish).
export const getHomepageSections = () => get("/api/homepage/sections").then((r) => r.sections || []);
export const adminGetHomepage = () => get("/api/admin/homepage").then((r) => r.sections || []);
export const adminPublishHomepage = (sections) => put("/api/admin/homepage", { sections }).then((r) => r.sections || []);
...
/* Admin variant — identical PDF blob download. No customer Bearer is present in the
   admin panel; the same-origin request carries the admin session cookie, which the
   invoice route also accepts. */
export const adminDownloadInvoice = (orderId) => downloadInvoice(orderId);
...
// Mark an order's invoice-updated notification as seen (customer, owner-only).
export const markInvoiceSeen = (id) => patch(`/api/orders/${id}/invoice-seen`, {}).then((r) => r.ok);
...
export const adminShipOrder = (id, serialNumbers = []) => post(`/api/admin/orders/${id}/ship`, { serialNumbers }).then((r) => r);
// Add/edit device serial numbers on an existing order without changing status.
// silent: true → save only, don't stamp/reset the invoice-updated notice or email.
export const adminUpdateSerials = (id, serialNumbers, message, silent = false) => patch(`/api/admin/orders/${id}/serials`, { serialNumbers, message, silent }).then((r) => r.order);
