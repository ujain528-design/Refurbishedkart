...
export function specRowsFor(product) {
  ...
  return [
    ...
    ["Generation", a.gen],
    ["RAM Type", a.ramType],
    ["RAM Expandability", a.ramExpandability],
    ...
    ["Form Factor", a.formFactor || a.chassis],
    ["Graphics (GPU)", a.gpu],
    ["Power Supply (PSU)", a.psu],
    ...
    // One battery row, never two: when batteryHealth is set, show only that
    // (the real per-unit value); otherwise the standardised backup guarantee.
    ["Battery", isLaptop && !a.batteryHealth ? "1.5hr+ Battery Backup Tested" : undefined],
...
export function descriptionFor(product) {
  ...
  const generated = [
    ...
    `Before listing, every unit clears our 32-point hardware inspection: chassis and hinge integrity, keyboard and trackpad wear, display uniformity, thermals under load, performance benchmarking, and storage health. ${
      product.category === "Laptops" ? "Every laptop's battery is tested for 1.5hr+ backup. " : ""
    }Each device is professionally cleaned and re-imaged with a genuine OS license.`,
...
const INSPECTION = [
  ...
  { key: "battery", name: "Battery Health", text: "1.5hr+ Battery Backup Tested." },
